...
def generate_stubs(keys: set[str], output_dir: Path):
    """Generates the config.pyi stub file with str types."""
    content_header = "# Autogenerated stub file for config module. Do not edit manually.\n\n"
    content = ((f"{key}: str\n", f"{key}: Path\n") for key in keys)
    with open(output_dir / "config.pyi", "w", encoding="utf-8") as f, open(
        output_dir / "paths.pyi", "w", encoding="utf-8"
    ) as p:
        f.write(content_header)
        p.write(content_header)
        p.write("from pathlib import Path\n\n")
        _ = [(f.write(line[0]), p.write(line[1])) for line in content]
        f.write("\n")
        p.write("\n")
    print(f"Generated: {output_dir / 'config.pyi'} and {output_dir / 'paths.pyi'}")
