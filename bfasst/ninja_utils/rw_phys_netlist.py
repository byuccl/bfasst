...
class PhysNetlistTransformError(Exception):
    """Exception for errors in the physical netlist transformation"""
...
class RwPhysNetlist:
    ...
    def run(self):
        ...
        rapdiwright_log_path = str(self.stage_dir / "rapidwright.log")
    ...
    def __run_rapidwright(self, phys_netlist_checkpoint, phys_netlist_edif_path):
        ...
        # This happens when we proces LUTS mapped to the same BEL
        visited_cells = set()

        # Loop through all sites and deal with LUTs. We can't use the later loop that iterates
        # over Design.getCells() because that doesn't include LUT routethru objects
        self.__process_all_luts(visited_cells)
        ...
        for cell in self.rw_design.getCells():
            ...
            logging.info(
                "%s %s",
                cell.getName(),
                edif_cell_inst.getCellType().getName() if edif_cell_inst else "None",
            )
            ...
            logging.error("ERROR: Handling for cell type %s not implemented", cell_type)
            raise PhysNetlistTransformError(f"Handling for cell type {cell_type} not implemented")

        logging.info("Removing old cells...")
        for cell in self.cells_to_remove:
            rw.remove_and_disconnect_cell(cell, logging.info)
    ...
    def __init_const_nets(self):
        ...
        for unisim_cell, port, name in net_tuples:
            ...
            # Create the new const instance as part of top-level
            ...
            const_port = const_edif_inst.getPortInst(port)
            assert const_port
            const_net.createPortInst(const_port, const_edif_inst)

    def __process_all_luts(self, visited_cells):
        ...
        for site_inst in self.rw_design.getSiteInsts():
            ...
            for lut6_bel, lut6_pin_out, lut5_bel, lut5_pin_out in self.lut_pair_bel_names:
                ...
                if lut6_cell and str(lut6_cell.getType()).startswith("RAM"):
                    ...
                    visited_cells.add(lut6_cell)

                    # Sanity check, pretty sure clk is not inverted when this value is one,
                    # so if there is a case where this changes, investigat the design to see.
                    if (
                        lut6_cell.getEDIFCellInst().getProperty("IS_CLK_INVERTED").getValue()
                        == "1'b1"
                    ):
                        # Rw sometimes report inverted clks when vivado shows them as not inverted
                        # logging.error("ERROR: LUT6 RAM clk inverted")
                        # raise PhysNetlistTransformError("ERROR: LUT6 RAM clk inverted")
                        logging.warning(
                ...
                if lut5_cell and str(lut5_cell.getType()).startswith("RAM"):
                    visited_cells.add(lut5_cell)
                    # Pretty sure this only happens when the LUT6 is a RAM
                    assert lut6_cell.getType() == lut5_cell.getType()
                    # Also opretty sure LUT5 cells are never combined? TBD if comparison fails
                    continue
            ...
            if len(lut_rams) > 1:
                self.__process_lutrams(lut_rams)
        visited_cells.remove(None)

    def __check_lut_gnd_nets(
        self, lut6_cell, lut6_pin_out, lut5_cell, lut5_pin_out, gnd_nets, site_inst
    ):
        ...
        if lut6_pin_out in gnd_nets:
            ...
            if lut5_cell is not None:
                self.__process_lut5_and_gnd_lut(lut5_cell, lut6_pin_out, site_inst)
                return {
                    lut5_cell,
                }
        ...
        if lut5_pin_out in gnd_nets:
            ...
            if lut6_cell is not None:
                self.__process_lut5_and_gnd_lut(lut6_cell, lut5_pin_out, site_inst)
                return {
                    lut6_cell,
                }
            gnd_generator_pins.append(lut5_pin_out)
        if gnd_generator_pins:
            self.__process_lut_gnd(site_inst, gnd_generator_pins)
        return set()

    def __process_lut5_and_gnd_lut(self, lut5, gnd_pin, site_inst):
        """Process a LUT5 and GND LUT pair."""
        assert lut5 is not None
        assert not lut5.isRoutethru()
        if str(lut5.getType()).startswith("RAM"):
            logging.error("ERROR: LUTRAM paired with gnd LUT not supported")
            raise PhysNetlistTransformError("LUTRAM paired with gnd LUT not supported")

        logging.info("")
        logging.info(
            "Processing and replcaing LUT5 %s paired with " + "GND LUT %s on site %s",
            lut5.getName(),
            gnd_pin,
            site_inst.getName(),
        )
        ...
        logging.info("Processing LUT %s", lut5.getName())
        for logical_pin, physical_pin in lut5.getPinMappingsL2P().items():
            ...
            self.__lut_move_net_to_new_cell(
        ...
        logging.info("Processing GND output pin %s", gnd_pin)
        logging.info("Creating new GND net %s", f"{site_inst.getName()}.{gnd_pin}.GND")

        new_net = EDIFNet(f"{site_inst.getName()}.{gnd_pin}.GND", self.rw_design.getTopEDIFCell())
        #  Drive net using LUT output port
        lut_out_port = new_cell_inst.getPort("O6" if gnd_pin.endswith("O6") else "O5")
        ...
        for physical_pin in site_inst.getSiteWirePins(gnd_pin):
            cell = site_inst.getCell(physical_pin.getBEL())
            if cell:
                logging.info(
                    " %s %s %s",
                    physical_pin,
        ...
        rw.process_shared_gnd_lut_eqn(lut5, gnd_pin, new_cell_inst, logging.info)
        return lut5

    def __lut_move_net_to_new_cell(
        self,
        old_edif_cell_inst,
        new_edif_cell_inst,
        old_logical_pin,
        ...
        logging.info("  Processing logical pin %s, physical pin %s", old_logical_pin, physical_pin)
        ...
        if already_connected_net:
            ...
            logging.info("    Skipping already connected physical pin %s", physical_pin)

        else:
            if port_inst.getDirection() == EDIFDirection.INPUT:
                logging.info("    Input driven by net %s", logical_net)

                # A5 becomes I4, A1 becomes I0, etc.
                new_logical_pin = f"I{int(str(physical_pin[1])) - 1}"
                logging.info(
                    "    Connecting net %s to input pin %s on new cell",
                    logical_net,
                    new_logical_pin,
                )

            elif port_inst.getDirection() == EDIFDirection.OUTPUT:
                logging.info("    Drives net %s", logical_net)

                new_logical_pin = physical_pin
                logging.info("    Connecting net %s to output pin %s", logical_net, new_logical_pin)
        ...
        logging.info("Disconnecting net %s from pin %s on old cell", logical_net, old_logical_pin)
        logical_net.removePortInst(port_inst)

    def __process_lut_gnd(self, site_inst, pins):
        """Process a LUT that isn't part of the design (ie no cell), but
        is configured to generate a GND signal"""

        logging.info("")
        logging.info(
            "Processing LUT GND at site %s, pin(s): %s", site_inst, ",".join(str(p) for p in pins)
        )
        ...
        for pin_out in pins:
            logging.info("Processing GND output pin %s", pin_out)

            # Create a new net to replace the global ground
            new_net_name = str(site_inst.getName()) + "." + pin_out + ".GND"
            logging.info("  Creating new GND net %s", new_net_name)
            ...
            for pin_in in site_inst.getSiteWirePins(pin_out):
                ...
                if cell:
                    logging.info(
                        " %s %s %s", pin_in, pin_in.getBEL(), site_inst.getCell(pin_in.getBEL())
                    )
                    routed_to_cell_inst = cell.getEDIFCellInst()
    ...
    def __process_lutrams(self, lut_rams):
        ...
        if not cell_type <= {"RAMS32", "RAMD32"}:
            logging.error("ERROR: Unexpected child cell type %s", cell_type)
        ...
        if len(cell_type) > 1:
            ...
            if lut_rams[0].getType() == lut_rams[1].getType():
                ...
                logging.error("ERROR: Unexpected lutram configuration in slice: %s", type_str)
    ...
    def __lutram_assertions(self, lut_rams):
        ...
        if not test_type < {"RAM32X1S", "RAM32X1D"}:
            logging.error("ERROR: Unexpected parent cell type %s", test_type)
    ...
    def __process_ram32m(self, cells):
        ...
        """
        if len(cells) == 2:
            logging.error("ERROR: RAM32D not handled yet")
    ...
    def __process_lut(self, lut6_cell, lut5_cell):
        ...
        if lut6_cell.isRoutethru():
            self.__create_lut_routethru_net(lut6_cell, False, new_cell_inst)
        if lut5_cell and lut5_cell.isRoutethru():
            self.__create_lut_routethru_net(lut5_cell, True, new_cell_inst)

        # Fix the new LUT INIT property based on the new pin mappings
        rw.process_lut_init(lut6_cell, lut5_cell, new_cell_inst, logging.info)
