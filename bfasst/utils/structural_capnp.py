...
class StructuralCapnp(RwPhysNetlist):
    ...
    def _check_bram(self, name: str, ecell: EDIFCellInst, rev_cell: Cell) -> set[str]:
        ...
        bram_do = ecell.getProperty("DOA_REG").getValue() == "0"
        if bram_do:
            assert ecell.getProperty("DOB_REG").getValue() == "0"
            ignore_pins.update(("RSTREGARSTREG", "RSTREGB", "REGCEAREGCE", "REGCEB"))
        bram_a_only = False
        if ecell.getProperty("RAM_MODE").getValue() == "TDP":
        ...
        # According to the BRAM Doc, if SDP, then these ports are not used.
        if ecell.getProperty("RAM_MODE").getValue() == "SDP":
            ignore_pins.update(
                ("CASCADEINA", "CASCADEINB", "CASCADEOUTA", "CASCADEOUTB", "REGCEB", "RSTREGB")
            )
    ...
    def _check_nets(self) -> None:
        ...
        for name, (ecell, rev_cell, _) in self.matches.items():
            ...
            if rev_cell.getType().startsWith("LUT"):
                self.update_lut_ports(rev_cell, rev_port_insts)
            elif rev_cell.getType().startsWith("RAMB"):
            ...
            for port in ecell_port_insts:
                ...
                if net_driver not in self.net_map:
                    logging.info(
                        "Mapping net %s to %s based on cell/port %s mapped to %s",
                        net_driver,
                        rev_net_driver,
                        str(port),
                        str(rev_port),
                ...
                # If the net is already mapped to a different rev net, raise an error
                    ...
                    # si = self.vivado_design.getSiteInst(rev_cell.getSiteInst().getSiteName())
                    # logging.info(
    ...
    def fix_rev_dsps(self) -> None:
        ...
        for cell in (c for c in self.rev_design.getCells() if c.getType() == "DSP48E1"):
            ...
            if [net for net in opmode if not rw.is_static_net(net)]:
                logging.info(
                    "Nonconstant OPMODE: Setting USE_MULT to DYNAMIC for %s", cell.getName()
                )
