...
def main():
    ap = argparse.ArgumentParser(
        description="Compute per-design fmax from physcmp logs (golden overall_wns)."
    )
    ap.add_argument(
        "--default_root",
        default="../build_default_vtr_new",
        help="Root of default phys_opt results",
    )
    ap.add_argument(
        "--custom_root", default="../build_custom_vtr_new", help="Root of custom phys_opt results"
    )
    ap.add_argument(
        "--constraint_ns", type=float, default=10.0, help="Constraint period used for reports (ns)"
    )
    ap.add_argument(
        "--out_csv", default="physopt_compare_out/fmax_summary.csv", help="Output CSV path"
    )
    args = ap.parse_args()
