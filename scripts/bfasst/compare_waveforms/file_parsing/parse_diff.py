...
def place_data(group, append):
    ...
    """Appends each individual value to the append variable"""
        ...
        for line in file:
            if data["past_definitions"] is False:
                ...
                if data["time"] != curr_time:
                    ...
                    binary_time = place_data(
                        [curr_time] * len(binary_name), binary_time
                    )
                    state = place_data(data["state"], state)
                    time = place_data([curr_time] * len(name), time)
                    curr_time = data["time"]
...
def init_unequivalent_data():
    ...
    """A function that initializes the data struct unequivalent data, a structure that keeps track of all data that is unequivalent at a specific time."""
