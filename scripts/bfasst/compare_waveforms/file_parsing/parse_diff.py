...
def check_diff(paths):
    """This function will run through all of the differences between the two VCD files and 
    then determine equivalence based upon the number of different lines."""
    is_equivalent = False
    for i in range(2):
        signal = []
        binary_signal = []
                        ...
                        if past_definitions is False:
                            if "$dumpvars" in line:
                                ...
                                if (
                                    ...
                                    returned_signal = parse_signals(line)
                                    if returned_signal[0] is "1":
                                        binary_signal.append(returned_signal[1])
                                        binary_state.append("0")
                                    else:
                                        signal.append(returned_signal[1])
                                        state.append("b0")
                        else:
                            if line[0] is "0" or line[0] is "1":
                                if line[1 : len(line) - 1] in binary_signal:
                                    j = binary_signal.index(line[1 : len(line) - 1])
                                    if binary_state[j] != line[0]:
                                        binary_state[j] = line[0]
                                        write.write(line)
                            elif " " in line:
                                if (
                                    ...
                                    if state[j] != line[0 : line.index(" ")]:
                                        state[j] = line[0 : line.index(" ")]
                                        write.write(line)
                            else:
                                write.write(line)
...
def parse_diff(paths):
    """This function reads through the diff.txt file generated by running a diff 
    function on the two vcd files (text representation of a wave file).
    Since the VCD files represent each signal as a symbol (IE: $, #, !, etc), this 
    script replaces those names with the actual signal names.
    This script also tells you the exact times, in ms, that these differences occur.
    The output of this script is saved in the waveform folder AND is printed out anytime 
