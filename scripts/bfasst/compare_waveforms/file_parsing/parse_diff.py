...
def parse_signals(line):
    """Is used in the header of a VCD file to find the names of every signal, it's corresponding symbol, and it's bitsize."""
    words = []
    newWord = False
    if "$var wire" in line:
        word = ""
...
def append_unequivalent_data(unequivalent_data, time_related_data, name, length, group):
    """Checks for any unequivalent data based upon the data's status at any given time."""
    t = 0
    h = 0
    for i, j in zip(time_related_data[0][group], time_related_data[1][group]):
        if i != j:
...
def check_duplicates(line, data, write):
    """Checks if the signal is the same as it was previously or if it has changed at a specific time."""
...
def past_initial_data(line, data):
    """Parses all of the data at the start of the files to set the initial values for every variable."""
...
def check_diff(paths):
    """This function will run through all of the differences between the two VCD files and then determine equivalence based upon the number of different states for each signal."""
    is_equivalent = False
    time_related_data = []
    for i in range(2):
        data = {}
    ...
    unequivalent_data = append_unequivalent_data(
        unequivalent_data, time_related_data, binary_name, len(binary_signal), 0
    )

    unequivalent_data = append_unequivalent_data(
        unequivalent_data, time_related_data, name, len(signal), 1
    )
    ...
    if len(unequivalent_data["impl"]) > 0:
        ...
        with paths["diff"].open("x") as output:
            output.write(
                f"Differences between implicit and reversed waveforms for design {paths['modules'][0]}\n\n\n"
            )
            for name, impl, reversed, time in zip(
                unequivalent_data["name"],
                unequivalent_data["impl"],
                unequivalent_data["reversed"],
                unequivalent_data["time"],
                ...
                output.write(
                    f"{time}ns, Signal {name}, Impl {impl}, Reversed {reversed}\n\n"
                )
                total = total + 1
            output.write(
