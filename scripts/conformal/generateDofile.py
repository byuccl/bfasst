...
def main():
	...
	vlog = [join(srcDir, f) for f in listdir(srcDir) if (isfile(join(srcDir, f)) and f.endswith(".v"))]

	vhdl = [join(srcDir, f) for f in listdir(srcDir) if (isfile(join(srcDir, f)) and f.endswith(".vhd"))]
	...
	with open(dofile, "w+") as f:
		...
		f.write("read library -Both -Replace -sensitive -Verilog /auto/fsh/crg3710/research/lattice_libraries/sb_ice_syn.v -nooptimize\n")
		...
		if vhdl:
			...
			f.write(" -vhdl -Golden -continuousassignment Bidirectional -nokeep_unreach -norangeconstraint\n")
		...
		if vlog:
			...
			# TODO see what these options do
			f.write(" -Verilog -Golden -continuousassignment Bidirectional -nokeep_unreach -norangeconstraint\n")
		...
		f.write("read design %s -Verilog -Revised -sensitive -continuousassignment Bidirectional -nokeep_unreach -nosupply\n" % extracted)
